...
class  ShippingMethodService
{
    ...
    {
        return collect($this->drivers)->flatMap(fn(ShippingDriverInterface $driver) => $driver->getServices()->toCollection())->pipe(fn($items) => ShippingServiceData::collect($items, DataCollection::class));
    }
    ...
    {
        return $this->getShippingService()->toCollection()->map(function (ShippingServiceData $shipping_service) use ($origin, $destination, $cart) {
            ...
            Cache::put(key: "shipping_data:{$shipping_data->hash}", value: $shipping_data, ttl: now()->addMinutes(15));
